...
# Calendar-year 2023 male G1/G2 schedule only.
# Jan-Mar comes from the 2022年度 special-race schedule;
# Apr-Dec comes from the revised 2023年度 special-race schedule.
# Pure girls G1 meetings are not configured. Girls races embedded in mixed G1 meetings
# (Pearl Cup / Keirin Festival Women's Throne) are excluded at race level.
G1_G2_MEETINGS_2023 = (
    GradeMeeting("高知", "74", "kochi", "G1", date(2023, 2, 23), date(2023, 2, 26)),
    GradeMeeting("別府", "86", "beppu", "G2", date(2023, 3, 18), date(2023, 3, 21)),
    GradeMeeting("平塚", "35", "hiratsuka", "G1", date(2023, 5, 2), date(2023, 5, 7)),
    GradeMeeting("岸和田", "56", "kishiwada", "G1", date(2023, 6, 13), date(2023, 6, 18)),
    GradeMeeting("函館", "11", "hakodate", "G2", date(2023, 7, 15), date(2023, 7, 17)),
    GradeMeeting("西武園", "26", "seibuen", "G1", date(2023, 8, 15), date(2023, 8, 20)),
    GradeMeeting("青森", "12", "aomori", "G2", date(2023, 9, 15), date(2023, 9, 18)),
    GradeMeeting("弥彦", "21", "yahiko", "G1", date(2023, 10, 19), date(2023, 10, 22)),
    GradeMeeting("小倉", "81", "kokura", "G1", date(2023, 11, 21), date(2023, 11, 26)),
    # GP series day 1. Only Young Grand Prix 2023 is G2 and belongs in this dataset.
    GradeMeeting("立川", "28", "tachikawa", "G2", date(2023, 12, 28), date(2023, 12, 28), 12, "ヤング"),
)
...
def collect(start: date, end: date, out_dir: Path, sleep_seconds: float) -> dict[str, object]:
    ...
    if start.year != 2023 or end.year != 2023:
        raise ValueError("this collector is intentionally restricted to calendar-year 2023")

    meetings = [m for m in G1_G2_MEETINGS_2023 if m.end >= start and m.start <= end]
    if not meetings:
        raise ValueError("no configured 2023 G1/G2 meetings overlap the requested window")
    ...
    summary = {
        "target": f"2023 calendar-year G1/G2 meetings, non-girls races, {start.isoformat()} to {end.isoformat()}",
        "start_date": start.isoformat(),
        "end_date": end.isoformat(),
        "captured_at_utc": datetime.now(timezone.utc).isoformat(),
        "source": "楽天Kドリームス 公開レース情報",
        "meeting_source": "KEIRIN.JP 2022年度/2023年度 特別競輪等開催日程",
        ...
        "definition_note": "Official calendar-year 2023 male G1/G2 meeting IDs only. Pure girls G1 meetings are not configured; girls races embedded in mixed meetings are excluded by race type and L-class detection. G3/GP/F1/F2 are not crawled. Young Grand Prix 2023 is retained as G2. Individual unavailable/cancelled/special pages are recorded and skipped while the annual run continues.",
    }
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Collect non-girls races from calendar-year 2023 G1/G2 meetings")
