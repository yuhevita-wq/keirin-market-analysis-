...
OUT = ROOT / 'data' / 'audits' / 'market_support_1_23_46_trifecta_multi_2023.json'
STAKE = 100
...
def main():
    ...
    for rid in sorted(gate):
        ...
        r1 = ranked[0]
        mids = [ranked[1], ranked[2]]
        tails = [ranked[3], ranked[5]]  # support ranks 4 and 6

        trio_sets = [(r1, m, t) for m in mids for t in tails]
        tickets = sorted({perm for s in trio_sets for perm in itertools.permutations(s, 3)})
        result = tuple(results[rid])
        ...
        if len(examples) < 10:
            examples.append({
                ...
                'trio_sets': [list(x) for x in trio_sets],
                'ticket_count': len(tickets),
                'result_order': list(result),
                'hit': hit,
                'winning_trifecta_odds': trifecta[rid].get(result) if hit else None,
            })

    stake = tickets_total * STAKE
    out = {
        'status': 'MARKET_SUPPORT_1_23_46_TRIFECTA_MULTI_2023',
        'year': 2023,
        'years_read': [2023],
        ...
        'formation': 'Market support rank 1 - ranks 2,3 - ranks 4,6, converted to trifecta full multi for each of the four 3-car sets.',
        'strategy_detail': 'Four 3-car sets per race; buy all 6 finish-order permutations of each set, 24 trifecta tickets/race.',
