...
def main():
    ...
    for rid,r in sorted(races.items(),key=lambda x:(x[1].get('race_date',''),x[0])):
        ...
        d=build_v8_16_f17(trio[rid],tf[rid],r.get('predicted_line_formation') or '',rt)
        if not d.get('buy'):
        ...
        row={'race_id':rid,'race_type':rt,'group':g,'hit':int(bool(wins)),'payout':payout,'tickets':n,'formation':d.get('formation'),'branch_policy':d.get('branch_policy')}
        rows.append(row);by[g].append(row)
    result={'scheme':'v8.16-F17','dataset':'2024Q1','population':population,'overall':summary(rows),'by_group':{g:{**summary(by[g]),'population':pop[g]} for g in GROUPS},'entry_fail_reasons':dict(fail),'final_rule':'PS_AB + H1_top >= 2*H1_second; unchanged F09 formation','selection_rule':'quarantine/no bet inherited from v8.15-F16'}
    result['acceptance']='PASS_Q1_PROFIT' if result['overall']['profit_yen']>0 else 'REJECT_Q1_NOT_PROFITABLE'
    print('V8_16_F17_Q1_BEGIN');print(json.dumps(result,ensure_ascii=False,indent=2));print('V8_16_F17_Q1_END')
