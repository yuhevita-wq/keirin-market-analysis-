...
def collect(start: date, end: date, out_dir: Path, sleep_seconds: float) -> dict[str, object]:
    """Collect 2024 races using the exact same parsing logic as the frozen 2025 collector."""
    if start.year != 2024 or end.year != 2024:
        raise ValueError("this collector is intentionally restricted to 2024")
    if start > end:
    ...
    for day in daterange(start, end):
        ...
        try:
            ...
            for ref in refs:
                # Keep the earliest discovery exactly as in the frozen 2025 collector.
    ...
    summary = {
        ...
        "oos_note": "2024 is an out-of-sample validation year. Collection/parsing logic is inherited unchanged from the frozen 2025 collector.",
    }
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Collect all exact-label S-class preliminary race cards for 2024")
    parser.add_argument("--start-date", default="2024-01-01")
    parser.add_argument("--end-date", default="2024-12-31")
    parser.add_argument("--out-dir", default="data/2024/s_class_yosen")
