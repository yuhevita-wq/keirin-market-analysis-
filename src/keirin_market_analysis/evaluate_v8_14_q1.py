...
def main():
 ...
 for rid,r in sorted(races.items(),key=lambda x:(x[1].get('race_date',''),x[0])):
  ...
  d=build_v8_14_f15(trio[rid],tf[rid],r.get('predicted_line_formation') or '',rt)
  if not d.get('buy'):
  ...
  row={'race_id':rid,'group':g,'hit':int(bool(wins)),'payout':payout,'tickets':n,'compressed':int(n<before),'formation':d.get('formation'),'before_anchor':d.get('formation_before_anchor'),'before_price':d.get('formation_before_price'),'anchor':d.get('H1_anchor')}
  rows.append(row);by[g].append(row)
 result={'scheme':'v8.14-F15','dataset':'2024Q1','population':population,'overall':summary(rows),'by_group':{g:{**summary(by[g]),'population':pop[g]} for g in GROUPS},'entry_fail_reasons':dict(fail)}
 result['acceptance']='PASS_Q1_PROFIT' if result['overall']['profit_yen']>0 else 'REJECT_Q1_NOT_PROFITABLE'
 print('V8_14_F15_Q1_BEGIN');print(json.dumps(result,ensure_ascii=False,indent=2));print('V8_14_F15_Q1_END')
 print('V8_14_F15_GENERAL_ROWS_BEGIN');print(json.dumps(by['GENERAL'],ensure_ascii=False,indent=2));print('V8_14_F15_GENERAL_ROWS_END')
