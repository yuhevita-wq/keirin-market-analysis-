...
BASE=Path('data/2025/s_class_yosen')
OUT=Path('data/audits/selected_136_146_2025.json')
STAKE=100

# Frozen 2023-derived gate. DO NOT RETUNE ON 2025.
...
def main():
    ...
    for r in trio_rows:
        ...
        if len(c)==3 and o>0:
            trios[str(r['race_id'])].append((c,o))
    ...
    selected=[]; race_hits=[]; total_stake=total_payout=0; hit_races=0
    bypat={str(p):{'tickets':0,'hits':0,'stake_yen':0,'payout_yen':0} for p in TARGET_PATTERNS}
    analyzable=0; rank1_miss=0; winner123=0
    ...
    for rid,rows in trios.items():
        ...
        winning_combos=set(paid[rid])
        winner_combo=next(iter(winning_combos)) if len(winning_combos)==1 else None
        ranks={c:i+1 for i,c in enumerate(ranked)}
        if winner_combo:
            wr=tuple(sorted(ranks.get(x,99) for x in winner_combo))
            if wr==(1,2,3): winner123+=1
            if 1 not in wr: rank1_miss+=1
        race_pay=0; race_hit=False
        ...
        selected.append({'race_id':rid,'hit':race_hit,'payout_yen':race_pay,'entropy':entropy,'top3_conc':top3_conc,'rank1_share':rank1_share,'p123':p123})
    ...
    out={
      'status':'SELECTED_136_146_2025_FROZEN_2023_GATE',
      'year':2025,
      'years_read':[2025],
      'gate_source':'Frozen from 2023 selected_136_146 analysis; no 2024/2025 retuning.',
      'thresholds':{'entropy_min':ENTROPY_MIN,'top3_conc_max':TOP3_CONC_MAX,'p123_max':P123_MAX,'rank1_share_min':RANK1_SHARE_MIN},
      'strategy':'Buy support-rank trios 1-3-6 and 1-4-6, 100 yen each, only when frozen 2023 gate passes.',
      'payout_method':'Actual published 3連複 payout_yen from 2025 payouts.csv.',
      'result':{
        'total_analyzable_races':analyzable,
        'selected_races':len(selected),
        'selection_rate_pct':100*len(selected)/analyzable if analyzable else None,
        'tickets':sum(d['tickets'] for d in bypat.values()),
        'hit_races':hit_races,
        'race_hit_rate_pct':100*hit_races/len(selected) if selected else None,
        ...
        'rank1_miss_races':rank1_miss,
        'rank1_miss_rate_pct':100*rank1_miss/len(selected) if selected else None,
        'winner_123_races':winner123,
        'winner_123_rate_pct':100*winner123/len(selected) if selected else None,
        'by_pattern':bypat,
      },
      'warning':'2025 final validation using frozen 2023 gate and unchanged two-ticket strategy. No 2025 tuning applied. 2026 not read.'
    }
