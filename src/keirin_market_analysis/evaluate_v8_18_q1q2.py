...
from v8_18_f19_semifinal_h_adaptive import build_v8_18_f19
...
def evaluate(label,data):
    ...
    rows=[]; by_group=defaultdict(list); by_type=defaultdict(list); pop_group=Counter(); pop_type=Counter(); fail=Counter(); population=0; selection_diag=0
    for rid,r in sorted(races.items(),key=lambda x:(x[1].get('race_date',''),x[0])):
        ...
        d=build_v8_18_f19(trio[rid],tf[rid],r.get('predicted_line_formation') or '',rt)
        ...
        row={'race_id':rid,'race_date':r.get('race_date'),'race_type':rt,'group':g,'special_subtype':special_subtype(rt) if g=='SPECIAL' else None,'hit':int(bool(wins)),'payout':payout,'tickets':n,'formation':d.get('formation'),'branch_policy':d.get('branch_policy')}
        rows.append(row); by_group[g].append(row); by_type[rt].append(row)
    return {'dataset':label,'population':population,'overall':summary(rows),'by_group':{g:{**summary(by_group[g]),'population':pop_group[g]} for g in GROUPS},'by_race_type':{rt:{**summary(by_type[rt]),'population':pop_type[rt]} for rt in sorted(pop_type)},'selection_diagnostic_races':selection_diag,'entry_fail_reasons':dict(fail)}


def main():
    q1=evaluate('2024Q1_DEVELOPMENT',load()); q2=evaluate('2024Q2_DEVELOPMENT_AFTER_OBSERVED',load_q2())
    result={'scheme':'v8.18-F19','status':'DEVELOPMENT_Q1Q2_AFTER_Q2_OBSERVED','semifinal_rule':'PS_AB + H_AB; H concentrated => first H1; H balanced => first top2 H; F09 second/third; protect H first pool during whole-rider price contraction','Q1':q1,'Q2':q2,'acceptance':{'Q1_profit_positive':q1['overall']['profit_yen']>0,'Q2_profit_positive':q2['overall']['profit_yen']>0,'SEMIFINAL_Q1_profit_positive':q1['by_group']['SEMIFINAL']['profit_yen']>0,'SEMIFINAL_Q2_profit_positive':q2['by_group']['SEMIFINAL']['profit_yen']>0},'next_validation':'DO_NOT_TUNE_ON_NEXT_VALIDATION_SET'}
    print('V8_18_F19_Q1Q2_BEGIN');print(json.dumps(result,ensure_ascii=False,indent=2));print('V8_18_F19_Q1Q2_END')
