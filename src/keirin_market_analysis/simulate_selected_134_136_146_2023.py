...
OUT=Path('data/audits/selected_134_136_146_2023.json')
STAKE=100

# Same frozen 2023 gate used in selected_136_146_2023. No retuning.
...
def main():
    ...
    analyzable=0; selected=0; hit_races=0; total_stake=0; total_payout=0; race_hits=[]
    ...
    out={
      'status':'SELECTED_134_136_146_2023_SAME_GATE',
      'year':2023,
      'years_read':[2023],
      'thresholds':{'entropy_min':ENTROPY_MIN,'top3_conc_max':TOP3_CONC_MAX,'p123_max':P123_MAX,'rank1_share_min':RANK1_SHARE_MIN},
      'strategy':'Buy support-rank trios 1-3-4, 1-3-6, and 1-4-6, 100 yen each, only when the same selected 136-146 gate passes.',
      ...
      'warning':'2023 development-year comparison only. Gate unchanged; only 1-3-4 was added to the existing 1-3-6 and 1-4-6 tickets.'
    }
