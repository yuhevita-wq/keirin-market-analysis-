...
def evaluate(label,data):
    ...
    for rid,r in sorted(races.items(),key=lambda x:(x[1].get('race_date',''),x[0])):
        ...
        d=build_v8_19_f20(trio[rid],tf[rid],r.get('predicted_line_formation') or '',rt)
...
def main():
    ...
    result={
        ...
        'special_rule':'Only S級初特選 active: PS_AB + H-state consistency; H concentrated=>1st H1, balanced=>1st top2 H; protect first pool. S級特選/S級初日特選 no-bet; S級選抜 remains diagnostic-only.',
        'Q1':q1,'Q2':q2,
        'acceptance':{
            'Q1_profit_positive':q1['overall']['profit_yen']>0,
            'Q2_profit_positive':q2['overall']['profit_yen']>0,
            'SPECIAL_Q1_profit_positive':q1['by_group']['SPECIAL']['profit_yen']>0,
            'SPECIAL_Q2_profit_positive':q2['by_group']['SPECIAL']['profit_yen']>0,
            'S級初特選_Q1_profit_positive':q1['by_race_type'].get('Ｓ級初特選',{}).get('profit_yen',0)>0,
            'S級初特選_Q2_profit_positive':q2['by_race_type'].get('Ｓ級初特選',{}).get('profit_yen',0)>0,
        },
        'next_validation':'DO_NOT_TUNE_ON_NEXT_VALIDATION_SET',
    }
    print('V8_19_F20_Q1Q2_BEGIN');print(json.dumps(result,ensure_ascii=False,indent=2));print('V8_19_F20_Q1Q2_END')
