...
def summarize(rows):
    bet_races = len(rows)
    hits = sum(to_int(r["hit"]) for r in rows)
    tickets = sum(to_int(r["final_bet_count"]) for r in rows)
    payout = sum(to_int(r["payout_yen"]) for r in rows)
    stake = tickets * STAKE
    return {
        "bet_races": bet_races,
        "hit_races": hits,
...
if __name__ == "__main__":
    ...
    from inspect_v6_4_d02_q1_random3 import main as inspect_v6_4_d02_q1_random3
