...
from v7_1_f02_market_efficiency import build_v7_1_f02
...
OUT = ROOT / "artifacts/v7_1_f02_2024q1"
...
def parse_lines(s):
    ...
    for raw in (s or "").strip().split("/"):
        ...
        if not all(x.strip().isdigit() for x in parts): return None
        out.append(tuple(int(x.strip()) for x in parts))
...
def main():
    ...
    for rid, r in sorted(races.items(), key=lambda kv: (kv[1].get("race_date", ""), kv[0])):
        ...
        attr["population"] += 1
        d = build_v7_1_f02(trio[rid], tf[rid], r.get("predicted_line_formation") or "")
        if not d.get("buy"):
        ...
        paid = payouts[rid]
        win = [t for t in tickets if t in paid]
        payout = sum(paid[t] for t in win)
        ...
        rows.append({
            ...
            "formation_mass": d["formation_mass"],
            "uniform_mass": d["uniform_mass"],
            "excess_mass": d["excess_mass"],
            "hit": int(bool(win)),
            "payout_yen": payout,
            "winning_ticket": ";".join("-".join(map(str, t)) for t in win),
        })
    ...
    summary = {
        "bet_races": len(rows),
        "hit_races": hits,
        "miss_races": len(rows)-hits,
        "hit_rate_pct": 100*hits/len(rows) if rows else None,
        "tickets": tickets,
        "avg_tickets_per_race": tickets/len(rows) if rows else None,
        ...
        "max_losing_streak": max_streak(rows),
    }
    result = {
        "scheme_version": SCHEME_VERSION,
        "dataset": DATASET,
        "entry_rule": "unchanged v6.1 pre-formation gate",
        "formation_rule": "maximize market_mass - ticket_count/210 among all market-ranked nested formations",
        "fixed_place_counts": False,
        "price_cut": False,
        "attrition": dict(attr),
        "entry_fail_reasons": dict(fail),
        "summary": summary,
        "size_pattern_distribution": dict(sorted(shape_sizes.items(), key=lambda kv: (-kv[1], kv[0]))),
    }
    OUT.mkdir(parents=True, exist_ok=True)
    (OUT / "v7_1_f02_2024q1_summary.json").write_text(json.dumps(result, ensure_ascii=False, indent=2), encoding="utf-8")
    if rows:
        with (OUT / "v7_1_f02_2024q1_races.csv").open("w", encoding="utf-8-sig", newline="") as f:
            w = csv.DictWriter(f, fieldnames=list(rows[0].keys())); w.writeheader(); w.writerows(rows)
    print("V7_1_F02_Q1_RESULT_BEGIN")
    print(json.dumps(result, ensure_ascii=False, indent=2))
    print("V7_1_F02_Q1_RESULT_END")
