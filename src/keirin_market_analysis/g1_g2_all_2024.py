...
# Calendar-year 2024 male G1/G2 schedule only.
# Jan-Mar comes from KEIRIN.JP 2023年度 schedule; Apr-Dec from 2024年度 schedule.
# Pure girls G1 meetings are not configured. Girls races embedded in mixed meetings are excluded.
G1_G2_MEETINGS_2024 = (
    GradeMeeting("岐阜", "43", "gifu", "G1", date(2024, 2, 9), date(2024, 2, 12)),
    GradeMeeting("取手", "23", "toride", "G2", date(2024, 3, 21), date(2024, 3, 24)),
    GradeMeeting("いわき平", "13", "iwakitaira", "G1", date(2024, 4, 30), date(2024, 5, 5)),
    GradeMeeting("岸和田", "56", "kishiwada", "G1", date(2024, 6, 11), date(2024, 6, 16)),
    GradeMeeting("松戸", "31", "matsudo", "G2", date(2024, 7, 13), date(2024, 7, 15)),
    GradeMeeting("平塚", "35", "hiratsuka", "G1", date(2024, 8, 13), date(2024, 8, 18)),
    GradeMeeting("宇都宮", "24", "utsunomiya", "G2", date(2024, 9, 13), date(2024, 9, 16)),
    GradeMeeting("弥彦", "21", "yahiko", "G1", date(2024, 10, 17), date(2024, 10, 20)),
    GradeMeeting("小倉", "81", "kokura", "G1", date(2024, 11, 19), date(2024, 11, 24)),
    GradeMeeting("静岡", "38", "shizuoka", "G2", date(2024, 12, 28), date(2024, 12, 28), 12, "ヤング"),
)
...
def collect(start: date, end: date, out_dir: Path, sleep_seconds: float) -> dict[str, object]:
    ...
    if start.year != 2024 or end.year != 2024:
        raise ValueError("this collector is intentionally restricted to calendar-year 2024")

    meetings = [m for m in G1_G2_MEETINGS_2024 if m.end >= start and m.start <= end]
    if not meetings:
        raise ValueError("no configured 2024 G1/G2 meetings overlap the requested window")
    ...
    summary = {
        "target": f"2024 calendar-year G1/G2 meetings, non-girls races, {start.isoformat()} to {end.isoformat()}",
        "start_date": start.isoformat(),
        "end_date": end.isoformat(),
        "captured_at_utc": datetime.now(timezone.utc).isoformat(),
        "source": "楽天Kドリームス 公開レース情報",
        "meeting_source": "KEIRIN.JP 2023年度/2024年度 特別競輪等開催日程",
        ...
        "definition_note": "Official calendar-year 2024 male G1/G2 meeting IDs only. Pure girls G1 meetings are not configured; girls races embedded in mixed meetings are excluded by race type and L-class detection. G3/GP/F1/F2 are not crawled. Young Grand Prix 2024 is retained as G2. Individual unavailable/cancelled/special pages are recorded and skipped while the annual run continues.",
    }
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Collect non-girls races from calendar-year 2024 G1/G2 meetings")
