...
def discover_g3_tracks_from_daily_html(html: str) -> set[str]:
    soup = BeautifulSoup(html, "lxml")
    text = base.normalize_text(soup.get_text(" ", strip=True))
    found: set[str] = set()
    # KDreams normally publishes the grade as full-width Ｇ３. Keep common
    # ASCII/Roman-numeral spellings too so discovery does not depend on typography.
    for match in re.finditer(r"([^\s]+?)(?:Ｇ３|G3|ＧⅢ|GIII)", text, flags=re.IGNORECASE):
        token = match.group(1)
...
def looks_like_race_type(label: str) -> bool:
    # Published KDreams race labels are class-prefixed. This covers S/A/L races,
    # including challenge and girls labels whose detailed names follow the prefix.
    return bool(re.match(r"^(?:Ｓ級|S級|Ａ級|A級|Ｌ級|L級)", label))


def discover_all_races_from_daily_html(html: str, daily_url: str, discovered_on: str) -> list[G3RaceRef]:
    soup = BeautifulSoup(html, "lxml")
    ...
    for table in soup.find_all("table"):
        ...
        for row_index, row in enumerate(rows):
            ...
            target_indexes = [i for i, label in enumerate(labels) if looks_like_race_type(label)]
...
def extract_entries_for_ref(html: str, ref: G3RaceRef):
    # The proven extractor validates the published race label against this module
    # global. Collection is intentionally serial here, so changing it is safe.
...
def collect(start: date, end: date, out_dir: Path, sleep_seconds: float) -> dict[str, object]:
    ...
    if start.year != 2024 or end.year != 2024:
        raise ValueError("this G3 capacity collector is intentionally restricted to 2024")
    ...
    g3_tracks_by_day: dict[str, set[str]] = {}
    ...
    for day in base.daterange(start, end):
        ...
        try:
            ...
            g3_tracks_by_day[day.isoformat()] = discover_g3_tracks_from_daily_html(html)
            for ref in discover_all_races_from_daily_html(html, daily_url, day.isoformat()):
    ...
    for ref in sorted(discovered.values(), key=lambda item: (item.discovered_on, item.url)):
        try:
            ...
            if track not in g3_tracks_by_day.get(ref.discovered_on, set()):
                skipped_non_g3 += 1
                continue
    ...
    summary = {
        ...
        "candidate_all_races": len(discovered),
        "parsed_g3_races": len(races),
        "entry_rows": len(entries),
        "skipped_non_g3": skipped_non_g3,
        ...
        "tracks": sorted({normalize_track_name(str(r["track"])) for r in races}),
        "definition_note": "楽天Kドリームス日別開催一覧で開催グレードがＧ３と明示された開催のみを対象とし、その開催内の全レースを保存する。級班・車立て数では絞らない。",
    }
