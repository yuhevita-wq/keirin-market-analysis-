...
from v8_2_f03_geometric_bridge import build_v8_2_f03

SCHEME="v8.2-F03"
ROOT=Path(__file__).resolve().parents[2]
OUT=ROOT/"artifacts/v8_2_f03_2024q1"
...
def main():
    ...
    for rid,r in sorted(races.items(),key=lambda kv:(kv[1].get("race_date",""),kv[0])):
        ...
        d=build_v8_2_f03(trio[rid],tf[rid],r.get("predicted_line_formation") or "")
        ...
        out.append({"race_id":rid,"race_date":r.get("race_date"),"track":r.get("track"),"race_no":pi(r.get("race_no")),"formation":d["formation"],"size_pattern":size,"ticket_count":d["ticket_count"],"bridge_mass":d["bridge_mass"],"cross_excess":d["cross_excess"],"q_mass":d["q_mass"],"q_star_mass":d["q_star_mass"],"hit":int(bool(wins)),"payout_yen":payout,"winning_ticket":";".join("-".join(map(str,t)) for t in wins)})
    ...
    result={"scheme_version":SCHEME,"dataset":"2024Q1","status":"Q1_DEVELOPMENT_SIMULATION","entry_rule":"unchanged v6.1 pre-formation gate","formation_rule":"independent rectangular F1-F2-F3; Pareto knee of geometric bridge sqrt(q*q_star) versus point count","fixed_place_counts":False,"fixed_point_count":False,"price_cut":False,"nested_required":False,"attrition":dict(attr),"entry_fail_reasons":dict(fail),"summary":{"bet_races":len(out),"hit_races":h,"miss_races":len(out)-h,"hit_rate_pct":100*h/len(out) if out else None,"tickets":n,"avg_tickets_per_race":n/len(out) if out else None,"min_tickets_per_race":min((r["ticket_count"] for r in out),default=None),"max_tickets_per_race":max((r["ticket_count"] for r in out),default=None),"stake_yen":s,"payout_yen":p,"profit_yen":p-s,"roi_pct":100*p/s if s else None,"max_losing_streak":streak(out)},"size_pattern_distribution":dict(sorted(sizes.items(),key=lambda kv:(-kv[1],kv[0]))),"top_20_exact_formations":dict(forms.most_common(20))}
    OUT.mkdir(parents=True,exist_ok=True);(OUT/"v8_2_f03_2024q1_summary.json").write_text(json.dumps(result,ensure_ascii=False,indent=2),encoding="utf-8")
    if out:
        with (OUT/"v8_2_f03_2024q1_races.csv").open("w",encoding="utf-8-sig",newline="") as f:
            w=csv.DictWriter(f,fieldnames=list(out[0].keys()));w.writeheader();w.writerows(out)
    print("V8_2_F03_Q1_RESULT_BEGIN");print(json.dumps(result,ensure_ascii=False,indent=2));print("V8_2_F03_Q1_RESULT_END")
