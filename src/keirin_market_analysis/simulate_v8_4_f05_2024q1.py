...
OUT=ROOT/"artifacts/v8_4_f05_2024q1"

def main():
    races,trio,tf,pay=load()
    out=[];fail=Counter();sizes=Counter();forms=Counter();steps=Counter();attr=defaultdict(int)
    ...
    for rid,r in sorted(races.items(),key=lambda kv:(kv[1].get("race_date",""),kv[0])):
        ...
        sizes[size]+=1;forms[d["formation"]]+=1;steps[str(d["selected_growth_step"])]+=1
        out.append({
            "race_id":rid,
            ...
            "q_mass":d["q_mass"],
            "bridge_mass":d["bridge_mass"],
            "q_star_mass":d["q_star_mass"],
            "cross_excess":d["cross_excess"],
            "hit":int(bool(wins)),
            "payout_yen":payout,
    ...
    h=sum(r["hit"] for r in out)
    n=sum(r["ticket_count"] for r in out)
    p=sum(r["payout_yen"] for r in out)
    s=n*STAKE
    result={
        ...
        "formation_rule":"start from strongest exact-order market center, add one rider at a time by marginal q per added ticket; cross-market bridge auxiliary; choose cumulative q-mass knee",
        ...
        "summary":{
            "bet_races":len(out),
            "hit_races":h,
            "miss_races":len(out)-h,
            "hit_rate_pct":100*h/len(out) if out else None,
            "tickets":n,
            "avg_tickets_per_race":n/len(out) if out else None,
        ...
        "selected_growth_step_distribution":dict(sorted(steps.items(),key=lambda kv:int(kv[0]))),
        "top_20_exact_formations":dict(forms.most_common(20)),
    }
    OUT.mkdir(parents=True,exist_ok=True)
    (OUT/"v8_4_f05_2024q1_summary.json").write_text(json.dumps(result,ensure_ascii=False,indent=2),encoding="utf-8")
    if out:
        with (OUT/"v8_4_f05_2024q1_races.csv").open("w",encoding="utf-8-sig",newline="") as f:
            w=csv.DictWriter(f,fieldnames=list(out[0].keys()));w.writeheader();w.writerows(out)
    print("V8_4_F05_Q1_RESULT_BEGIN")
    print(json.dumps(result,ensure_ascii=False,indent=2))
    print("V8_4_F05_Q1_RESULT_END")
