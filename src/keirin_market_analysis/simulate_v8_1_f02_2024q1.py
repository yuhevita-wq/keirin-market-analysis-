...
from v8_1_f02_cross_market_rectangular import build_v8_1_f02

SCHEME = "v8.1-F02"
STAKE = 100
ROOT = Path(__file__).resolve().parents[2]
DATA = ROOT / "data/2024/s_class_f1_all_parts/2024_q1"
OUT = ROOT / "artifacts/v8_1_f02_2024q1"
...
def main():
    ...
    for rid, r in sorted(races.items(), key=lambda kv: (kv[1].get("race_date", ""), kv[0])):
        ...
        d = build_v8_1_f02(trio[rid], tf[rid], r.get("predicted_line_formation") or "")
        if not d.get("buy"):
        ...
        out.append({
            ...
            "positive_excess": d["positive_excess"],
            "negative_excess": d["negative_excess"],
            "q_mass": d["q_mass"],
    ...
    result = {
        ...
        "formation_rule": "independent rectangular F1-F2-F3; Pareto knee of summed cross-market excess versus point count",
        "cross_market_excess": "q_star(t)-q(t), where q_star(t)=P_trio(c)*q(t)/Q_trifecta_trio(c)",
        ...
        "top_20_exact_formations": dict(formations.most_common(20)),
    }

    OUT.mkdir(parents=True, exist_ok=True)
    (OUT / "v8_1_f02_2024q1_summary.json").write_text(json.dumps(result, ensure_ascii=False, indent=2), encoding="utf-8")
    if out:
        with (OUT / "v8_1_f02_2024q1_races.csv").open("w", encoding="utf-8-sig", newline="") as f:
            w = csv.DictWriter(f, fieldnames=list(out[0].keys()))
            w.writeheader()
            w.writerows(out)

    print("V8_1_F02_Q1_RESULT_BEGIN")
    print(json.dumps(result, ensure_ascii=False, indent=2))
    print("V8_1_F02_Q1_RESULT_END")
