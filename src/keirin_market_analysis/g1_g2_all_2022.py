...
# Calendar-year 2022 male G1/G2 schedule only.
# Jan-Mar comes from KEIRIN.JP 2021年度 schedule; Apr-Dec from 2022年度.
# Girls races embedded in mixed meetings are excluded at race level.
G1_G2_MEETINGS_2022 = (
    GradeMeeting("取手", "23", "toride", "G1", date(2022, 2, 20), date(2022, 2, 23)),
    GradeMeeting("宇都宮", "24", "utsunomiya", "G2", date(2022, 3, 18), date(2022, 3, 21)),
    GradeMeeting("いわき平", "13", "iwakitaira", "G1", date(2022, 5, 3), date(2022, 5, 8)),
    GradeMeeting("岸和田", "56", "kishiwada", "G1", date(2022, 6, 16), date(2022, 6, 19)),
    GradeMeeting("玉野", "61", "tamano", "G2", date(2022, 7, 16), date(2022, 7, 18)),
    GradeMeeting("西武園", "26", "seibuen", "G1", date(2022, 8, 9), date(2022, 8, 15)),
    GradeMeeting("名古屋", "42", "nagoya", "G2", date(2022, 9, 16), date(2022, 9, 19)),
    GradeMeeting("前橋", "22", "maebashi", "G1", date(2022, 10, 20), date(2022, 10, 23)),
    GradeMeeting("小倉", "81", "kokura", "G1", date(2022, 11, 22), date(2022, 11, 27)),
    # GP series day 1: only Young Grand Prix 2022 is G2.
    GradeMeeting("平塚", "35", "hiratsuka", "G2", date(2022, 12, 28), date(2022, 12, 28), 12, "ヤング"),
)
...
def collect(start: date, end: date, out_dir: Path, sleep_seconds: float) -> dict[str, object]:
    ...
    if start.year != 2022 or end.year != 2022:
        raise ValueError("this collector is intentionally restricted to calendar-year 2022")

    meetings = [m for m in G1_G2_MEETINGS_2022 if m.end >= start and m.start <= end]
    if not meetings:
        raise ValueError("no configured 2022 G1/G2 meetings overlap the requested window")
    ...
    summary = {
        "target": f"2022 calendar-year G1/G2 meetings, non-girls races, {start.isoformat()} to {end.isoformat()}",
        "start_date": start.isoformat(),
        "end_date": end.isoformat(),
        "captured_at_utc": datetime.now(timezone.utc).isoformat(),
        "source": "楽天Kドリームス 公開レース情報",
        "meeting_source": "KEIRIN.JP 2021年度/2022年度 特別競輪等開催日程",
        ...
        "definition_note": "Official calendar-year 2022 male G1/G2 meeting IDs only. Girls races embedded in mixed meetings are excluded by race type and L-class detection. G3/GP/F1/F2 are not crawled. Young Grand Prix 2022 is retained as G2. Individual unavailable/cancelled/special pages are recorded and skipped while the annual run continues.",
    }
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Collect non-girls races from calendar-year 2022 G1/G2 meetings")
