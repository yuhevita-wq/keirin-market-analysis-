...
def discover_f1_tracks_from_daily_html(html: str) -> set[str]:
    """Return track names whose meeting grade is published as F1 on the daily page."""
    soup = BeautifulSoup(html, "lxml")
    text = base.normalize_text(soup.get_text(" ", strip=True))
    # KDreams daily pages publish meeting names such as '西武園Ｆ１' in 開催一覧.
    # Track names contain Japanese characters; stop at whitespace before the grade marker.
    found = set()
    for match in re.finditer(r"([^\s]+?)Ｆ１", text):
        token = match.group(1)
        # Avoid swallowing headings/punctuation by keeping the final contiguous name fragment.
...
def discover_s_class_from_daily_html(html: str, daily_url: str, discovered_on: str) -> list[SClassRaceRef]:
    """Return all published race-card links whose race label starts with full-width 'Ｓ級'."""
    soup = BeautifulSoup(html, "lxml")
...
def extract_entries_for_ref(html: str, ref: SClassRaceRef):
    # Reuse the frozen parser while supplying the actually published S-class label.
...
def collect(start: date, end: date, out_dir: Path, sleep_seconds: float) -> dict[str, object]:
    ...
    race_fields = [
        "race_id", "race_date", "track", "meeting_grade", "race_no", "race_type", "start_time",
    ...
    summary = {
        "target": "2023 F1 meetings, all S-class races",
        ...
        "entry_count_counts": entry_count_counts,
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Collect all S-class races from 2023 F1 meetings")
    parser.add_argument("--start-date", default="2023-01-01")
    parser.add_argument("--end-date", default="2023-12-31")
    parser.add_argument("--out-dir", default="data/2023/s_class_f1_all")
