...
def evaluate(label,data):
    ...
    pop_group=Counter(); pop_type=Counter(); fail=Counter(); population=0; selection_diag=0
    ...
    return {
        ...
        'selection_diagnostic_races':selection_diag,'entry_fail_reasons':dict(fail)
    }
...
def main():
    ...
    result={
        ...
        'correction':'S級初特選 and S級初日特選 are treated as the same INITIAL_SPECIAL race regime.',
        'initial_special_rule':'Both labels: PS_AB + H-state consistency; H concentrated=>1st H1, balanced=>1st top2 H; protect first pool.',
        'Q1':q1,'Q2':q2,
        'acceptance':{
            'Q1_profit_positive':q1['overall']['profit_yen']>0,
            'Q2_profit_positive':q2['overall']['profit_yen']>0,
            'INITIAL_SPECIAL_Q1_profit_positive':q1['INITIAL_SPECIAL_UNIFIED']['profit_yen']>0,
            'INITIAL_SPECIAL_Q2_profit_positive':q2['INITIAL_SPECIAL_UNIFIED']['profit_yen']>0,
        },
        'next_validation':'DO_NOT_TUNE_ON_NEXT_VALIDATION_SET'
    }
    print('V8_20_F21_Q1Q2_BEGIN');print(json.dumps(result,ensure_ascii=False,indent=2));print('V8_20_F21_Q1Q2_END')
