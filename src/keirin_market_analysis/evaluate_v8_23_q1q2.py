...
def evaluate(label,data):
    ...
    rows=[];by_group=defaultdict(list);by_type=defaultdict(list);initial=[];selection=[];pop_group=Counter();pop_type=Counter();fail=Counter();population=0;final_quarantine=0
    for rid,r in sorted(races.items(),key=lambda x:(x[1].get('race_date',''),x[0])):
        ...
        d=build_v8_23_f24(trio[rid],tf[rid],r.get('predicted_line_formation') or '',rt)
        if rt=='Ｓ級決勝' and d.get('branch_policy')=='FINAL_HEAT_QUARANTINE_DIAGNOSTIC_ONLY':final_quarantine+=1
        if not d.get('buy'):
    ...
    return {
        ...
        'final_quarantined_population':final_quarantine,'entry_fail_reasons':dict(fail)
    }

def main():
    q1=evaluate('2024Q1_DEVELOPMENT',load());q2=evaluate('2024Q2_DEVELOPMENT_AFTER_OBSERVED',load_q2())
    result={'scheme':'v8.23-F24','status':'DEVELOPMENT_Q1Q2_SELECTION_ULTRA_ROI','change_from_v8_22':'SELECTION only: H balanced; most negative-D set among trio top3; target includes global H1 and spans exactly 2 lines; head=highest H in target; buy higher-odds tail order only (1 ticket). All other branches unchanged.','Q1':q1,'Q2':q2,'acceptance':{'Q1_profit_positive':q1['overall']['profit_yen']>0,'Q2_profit_positive':q2['overall']['profit_yen']>0},'next_validation':'Q3_UNTOUCHED_ONLY_AFTER_FREEZE'}
    print('V8_23_F24_Q1Q2_BEGIN');print(json.dumps(result,ensure_ascii=False,indent=2));print('V8_23_F24_Q1Q2_END')
