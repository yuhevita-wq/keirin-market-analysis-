...
def pair_stats(pair, entrants, delta, trio):
    ...
    return {'positive_count':len(pos),'positive_sum':sum(pos),'min_delta':min(vals) if vals else float('-inf'),'mean_ptrio':sum(ptrio)/len(ptrio) if ptrio else 0.0}
...
def main():
    ...
    out={'status':'PAIR_SUPPORT_RANK_AXIS_2023','year':2023,'years_read':[2023],'population':'Audited 208 fake-favorite races only.','pair_support_definition':'Rank all actual entrant pairs by positive_count, then positive_sum, then min_delta, then mean trio implied mass, then lower car-number tuple.','strategy':'Use pair support rank #1, #2, or #3 directly as the two-head axis; literal full flow to every other entrant.','results':{},'note':'Development simulation only. No 2024/2025/2026 data read.'}
...
# trigger corrected pair-rank simulation
