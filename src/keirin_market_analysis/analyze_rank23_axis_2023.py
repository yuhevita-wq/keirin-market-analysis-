...
def main():
    ...
    acc=defaultdict(int)
    rank1_top3=rank2_top3=rank3_top3=0
    rank1_with_axis_hit=rank1_with_axis_total=0
    for rid in sorted(gate):
        ...
        if set(pair).issubset(rset):
            rank1_with_axis_total+=1; rank1_with_axis_hit+=int(r1 in rset)
        if hit: acc['payout']+=round(STAKE*trio[rid][result])
    stake=acc['tickets']*STAKE; payout=acc['payout']
    out={'status':'RANK23_AXIS_2023','year':2023,'years_read':[2023],
      ...
      'result':{'bet_races':acc['bet_races'],'tickets':acc['tickets'],'hit_races':acc['hit_races'],
        'race_hit_rate_pct':100*acc['hit_races']/acc['bet_races'],'axis_pair_survival_pct':100*acc['axis_survived']/acc['bet_races'],
