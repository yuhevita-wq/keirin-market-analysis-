...
from v7_6_f07_directional_coverage_knee import build_v7_6_f07
SCHEME="v7.6-F07";STAKE=100;ROOT=Path(__file__).resolve().parents[2];DATA=ROOT/"data/2024/s_class_f1_all_parts/2024_q1"
def rr(n):
...
def main():
 ...
 for rid,r in sorted(races.items(),key=lambda kv:(kv[1].get('race_date',''),kv[0])):
  ...
  d=build_v7_6_f07(trio[rid],tf[rid],r.get('predicted_line_formation') or '')
  ...
  out.append({'race_id':rid,'race_date':r.get('race_date'),'track':r.get('track'),'race_no':pi(r.get('race_no')),'formation':d['formation'],'size_pattern':size,'ticket_count':d['ticket_count'],'core_mass_coverage':d['core_mass_coverage'],'covered_core_tickets':d['covered_core_tickets'],'core_ticket_count':d['core_ticket_count'],'hit':int(bool(win)),'payout_yen':payout,'winning_ticket':';'.join('-'.join(map(str,t)) for t in win)})
 h=sum(r['hit'] for r in out);n=sum(r['ticket_count'] for r in out);p=sum(r['payout_yen'] for r in out);s=n*STAKE
 result={'scheme_version':SCHEME,'dataset':'2024Q1','entry_rule':'unchanged v6.1 pre-formation gate','formation_rule':'clean nested formation at Pareto knee of old-directional-core q-mass coverage versus total ticket cost','fixed_place_counts':False,'price_cut':False,'attrition':dict(attr),'entry_fail_reasons':dict(fail),'summary':{'bet_races':len(out),'hit_races':h,'miss_races':len(out)-h,'hit_rate_pct':100*h/len(out) if out else None,'tickets':n,'avg_tickets_per_race':n/len(out) if out else None,'min_tickets_per_race':min((r['ticket_count'] for r in out),default=None),'max_tickets_per_race':max((r['ticket_count'] for r in out),default=None),'stake_yen':s,'payout_yen':p,'profit_yen':p-s,'roi_pct':100*p/s if s else None,'max_losing_streak':streak(out)},'size_pattern_distribution':dict(sorted(sizes.items(),key=lambda kv:(-kv[1],kv[0])))}
 print('V7_6_F07_Q1_RESULT_BEGIN');print(json.dumps(result,ensure_ascii=False,indent=2));print('V7_6_F07_Q1_RESULT_END')
